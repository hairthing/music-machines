...
function Machine() {
  ...
  this.performerOutputQueue = async.queue(this.sendCommand.bind(this), 1);
  this.performerOutputQueue.pause();
...
Machine.prototype.connect = function(port) {
  this.serial = new SerialPort.SerialPort("/dev/cu.usbserial-A603G6VO", {baudrate: 115200, autoOpen: false}); // false == do not open immediately
  this.serial.on('data', this._handleSerialData.bind(this));
  this.serial.open(function(err) {
    if (err) { console.log(err); }
    this.emit("connected");
    this.machineIsConnected = true;
    this.pushCommand("G90");
  }.bind(this));
};
...
Machine.prototype.sendCommand = function(commandObject, callback) {
  ...
  this.serial.write(commandObject.command + "\n", function(err, results) {
    if (err) { console.log(err); }
    console.log("writ");
...
Machine.prototype.goTo = function(mm) {
  this.performerOutputQueue.push({command: "G0 X"+parseInt(mm,10)});
};

Machine.prototype.startPerforming = function() {
  this.currentLineNumber = 0;
  // this.performerOutputQueue.pause();
  // this.commandOutputQueue.pause();
  // this.currentGcodeFile.split("\n").forEach(function(command, line) {
  //   this.performerOutputQueue.push({command: command, line: line});
  // }.bind(this));
  this.activeQueue = this.performerOutputQueue;
  this.performerOutputQueue.resume();
...
//   return !this.performerOutputQueue.paused;
// };
//
